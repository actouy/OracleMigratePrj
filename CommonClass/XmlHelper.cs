...
namespace Common
{
    public class XmlHelper
    {
        ...
        /// <returns>写入的数据字符串</returns>
        static ReaderWriterLockSlim LogWriteLock = new ReaderWriterLockSlim();
        public static string WriteXML<T>(T item, string path)
        {
            int i = 0;//控制写入文件的次数，
            XmlSerializer serializer = new XmlSerializer(item.GetType());
            string xmlPath = path;
            while (true)
            {
                try
                {
                    ...
                    if (!File.Exists(xmlPath))
                    {
                        ...
                        FileStream fs;
                        fs = File.Create(xmlPath);//用filestream方式创建文件不会出现“文件正在占用中，用File.create”则不行
                        fs.Close();
                        TextWriter writer = new StreamWriter(xmlPath, false, Encoding.UTF8);
                        XmlSerializerNamespaces xml = new XmlSerializerNamespaces();
                        xml.Add(string.Empty, string.Empty);
                    ...
                    else
                    {
                        ...
                        FileStream stream = new FileStream(xmlPath, FileMode.Append);
                        ...
                        serializer.Serialize(xmlwriter, item, xmlns);
